...
import { InventoryBulkImportFailure, InventoryBulkImportResult, InventoryDuplicateMatch, inventoryApi } from '../../api/modules';
import type { ValidatedInventoryRow } from '../../utils/inventoryExcel';
import { downloadInventoryImportReport, parseInventoryWorkbook, unitLabel, validateInventoryRows } from '../../utils/inventoryExcel';
...
export function InventoryBulkImportModal({
  open,
  onClose,
  projectOptions,
  costCodeOptions,
  ...
  const [rows, setRows] = useState<ValidatedInventoryRow[]>([]);
  const [duplicates, setDuplicates] = useState<InventoryDuplicateMatch[]>([]);
  const [checkingDuplicates, setCheckingDuplicates] = useState(false);
  const [result, setResult] = useState<InventoryBulkImportResult | null>(null);
  const [combinedFailures, setCombinedFailures] = useState<InventoryBulkImportFailure[]>([]);
  ...
  const handleFileSelect = async (file: File) => {
    ...
    setParsing(true);
    try {
      const rawRows = await parseInventoryWorkbook(file);
      const validated = validateInventoryRows(rawRows, {
  ...
  const handleImportClick = async () => {
    ...
    try {
      const matches = await inventoryApi.checkDuplicates(
        validRows.map((r) => ({ projectId: r.projectId!, costCodeId: r.costCodeId!, itemDescription: r.itemDescription }))
      );
      if (matches.length > 0) {
      ...
      toast.error(apiErrorMessage(err, 'Could not check for duplicate inventory items'));
    } finally {
      setCheckingDuplicates(false);
    }
  };
    ...
    <Modal open={open} onClose={handleClose} title="Bulk Import from Excel" size="xl">
      {step === 'upload' && (
        <div>
          <div className="rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-900/40">
            ...
            <p className="text-sm font-medium">Upload a completed Inventory Template</p>
      ...
      {step === 'duplicates' && (
        <div>
          <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-900/20 dark:text-amber-300">
            ...
            <span>
              The following {duplicates.length} item{duplicates.length === 1 ? '' : 's'} already exist{duplicates.length === 1 ? 's' : ''} (matched
              by Project + Cost Code + Item Description). How should they be handled?
            </span>
          </div>
          ...
          <div className="mt-3 max-h-48 overflow-auto rounded-lg border border-slate-200 dark:border-slate-800">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {duplicates.map((d) => (
                  <tr key={d.id}>
                    <td className="px-3 py-2">{d.costCode?.code}</td>
                    <td className="px-3 py-2">{d.itemDescription}</td>
                    <td className="px-3 py-2">{d.project?.projectName}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="mt-3 text-xs text-slate-500">
            {validRows.length - duplicates.length} new item{validRows.length - duplicates.length === 1 ? '' : 's'} will be imported regardless of
            your choice below.
          </p>
      ...
      {step === 'importing' && (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="mt-3 text-sm text-slate-500">Importing inventory...</p>
        </div>
      )}
        ...
        <div>
          ...
          {combinedFailures.length > 0 && (
            <div className="mt-4 flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-800 dark:bg-slate-900/40">
              ...
              <button className="btn-secondary" onClick={() => downloadInventoryImportReport(combinedFailures)}>
